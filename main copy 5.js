...
const levelUpTitleMaterial = new THREE.MeshStandardMaterial({
  ...
  aoMap: boardLightMap,
  aoMapIntensity: 1,
  emissive: 0x8f008f, // Even lighter orange for emissive
  emissiveIntensity: 0.6, // Adjust emissive intensity
  normalScale: new THREE.Vector2(1, 1),
});
...
const tableLeg1 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.5, 0.2), woodMaterial);
const tableLeg2 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.5, 0.2), woodMaterial);
const tableLeg3 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.5, 0.2), woodMaterial);
const tableLeg4 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.5, 0.2), woodMaterial);
...
gltfLoader.load(
    ...
    (gltf) => {
        ...
        model.name = 'Lvlup Title'; // Assign a name for easy reference
        scene.add(model);
        populateObjectList(); // Refresh UI list after adding the plant
    },
    undefined,
    function (error) {
...
fbxLoader.load(
    ...
    (fbx) => {
        ...
        fbx.position.set(6.3, -2.5, 9.0); // Position the plant near the front wall
        ...
        fbx.name = 'Plant'; // Assign a name for easy reference
        scene.add(fbx);
        populateObjectList(); // Refresh UI list after adding the plant
    },
    (xhr) => {
...
class CounterPusher {
    constructor(loader, scene, position, rotation, name = 'Counter Pusher') {
        this.loader = loader;
        this.scene = scene;
        ...
        this.loader.load(
            ...
            (gltf) => {
                ...
                const customMaterial = new THREE.MeshStandardMaterial({
                    color: 0xffa700, // Bright orange color
                    metalness: 1.0, // Fully metallic
                    roughness: 0.2, // Slightly rough for reflection details
                    emissive: 0xffa07a, // Add emissive property for glow
                ...
                addCounterPusherToUI(this.name); // Display Counter Pusher name in UI
            },
            undefined,
            (error) => {
...
// Instantiate and load multiple Counter Pushers
const counterPusherPositions = [
    { x: 0, y: -0.5, z: -1.5 },
    { x: -2, y: -0.5, z: -1.5 },
    { x: -1.5, y: -0.5, z: -1.5 },
    { x: -1, y: -0.5, z: -1.5 },
...
const counterPusherRotations = { x: Math.PI, y: 0, z: Math.PI };

counterPusherPositions.forEach((position, index) => {
    const counterPusher = new CounterPusher(
        gltfLoader,
        scene,
        position,
        counterPusherRotations,
        `Counter Pusher ${index + 1}`
    );
    counterPusher.loadModel();
});
...
// Update objectList style to include a scroll bar
objectList.style.height = '300px'; // Set a fixed height
objectList.style.overflowY = 'scroll'; // Enable vertical scrolling
...
function populateObjectList() {
    ...
    scene.children.forEach((child) => {
        ...
        const existingEntry = [...objectList.children].find(
            (entry) => entry.textContent.includes(child.name)
        );
        if (existingEntry) return;

        if (child.isMesh || child.name.startsWith('Counter Pusher') || child.name === 'Plant' || child.name === 'Lvlup Title') {
...
// Add this variable to track the animation direction and speed
...
function animate(timestamp) {
    ...
    try {
        ...
        // Animate all Counter Pushers
        counterPusherPositions.forEach((position, index) => {
            const counterPusher = scene.getObjectByName(`Counter Pusher ${index + 1}`);
            if (counterPusher) {
